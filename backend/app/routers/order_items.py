...
def create_item(order_id: int, data: OrderItemCreate, db=Depends(get_db), _=Depends(get_current_user)):
    _, order = _get_order_ns(db, order_id)
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
...
def update_item(order_id: int, item_id: int, data: OrderItemUpdate, db=Depends(get_db), _=Depends(get_current_user)):
    _, order = _get_order_ns(db, order_id)
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    if order.status == "finalized":
...
def update_item_extra(
    order_id: int, item_id: int, data: OrderItemExtraUpdate,
    db=Depends(get_db), _=Depends(get_current_admin),
):
    ref = db.collection("order_items").document(str(item_id))
    doc = ref.get()
    if not doc.exists or doc.to_dict().get("daily_order_id") != order_id:
...
def cancel_item(order_id: int, item_id: int, db=Depends(get_db), _=Depends(get_current_user)):
    _, order = _get_order_ns(db, order_id)
    if not order:
        raise HTTPException(status_code=404, detail="Order not found")
    if order.status == "finalized":
